...
**Protective throttle (circuit breaker).** If a target starts blocking en masse
(a high rate of `403/429/451/503`), the router automatically *slows the pace of
new requests* to avoid burning your whole IP pool against a wall. It never
removes a backend — it's reversible and releases automatically once the failure
rate drops. It's conservative by default (trips at 20 failures in 60s) so it
won't interfere with normal work, and can be tuned or disabled:

```bash
SMART_ROUTER_CIRCUIT=off python3 middleware/smart_router.py ...   # disable entirely
SMART_ROUTER_CIRCUIT_THRESHOLD=40 ...   # trip only after 40 failures/60s
SMART_ROUTER_CIRCUIT_THROTTLE=1.0 ...   # smaller slow-down when tripped
```
> If you run heavy fuzzing/brute-force where many 4xx are expected by design,
> either raise the threshold or set `SMART_ROUTER_CIRCUIT=off`. Watch
> `smart_router_burn_rate` / `smart_router_circuit_throttling` in Grafana to see
> when it engages.
