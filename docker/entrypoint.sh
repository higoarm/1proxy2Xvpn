...
error() { echo -e "${R}[1proxy2Xvpn][ERROR]${N} $*" >&2; }
die()   { error "$*"; exit 1; }
...
# Close IPv6 immediately at boot too — before the tunnel exists — so there's no
# window where IPv6 traffic could leak outside the (IPv4-only) VPN tunnel.
...
# Wrapped in a function so the monitor loop can relaunch it if it dies.
launch_openvpn() {
    touch "${OPENVPN_LOG}"
    openvpn \
        --config "${OVPN_FILE}" \
...
# ── Monitor loop ──────────────────────────────────────────────────────────────
# Restart budget for OpenVPN: relaunch up to N times before giving up and letting
# Docker's restart policy recreate the container from scratch.
OVPN_RESTARTS=0
OVPN_MAX_RESTARTS="${OVPN_MAX_RESTARTS:-10}"
while true; do
    ...
    if [ -z "${OVPN_PID_VAL}" ] || ! kill -0 "${OVPN_PID_VAL}" 2>/dev/null; then
        # Kill Switch stays active the whole time, so there is never a leak while
        # OpenVPN is down. We now also try to bring the tunnel back up instead of
        # leaving the container alive-but-VPN-less.
        warn "OpenVPN died — Kill Switch active (no leak). Attempting restart..."
        # Reassert the Kill Switch defensively, then relaunch OpenVPN.
        /usr/local/bin/iptables_killswitch.sh "${VPN_IP}" "${VPN_PORT}" "${VPN_PROTO}" 2>/dev/null || true
        OVPN_RESTARTS=$((OVPN_RESTARTS + 1))
        if [ "${OVPN_RESTARTS}" -le "${OVPN_MAX_RESTARTS}" ]; then
            rm -f "${OPENVPN_PID}" 2>/dev/null || true
            launch_openvpn
            log "OpenVPN relaunched (attempt ${OVPN_RESTARTS}/${OVPN_MAX_RESTARTS})"
            # Give it a moment to come up before the next monitor cycle.
            sleep 10
        else
            error "OpenVPN failed to stay up after ${OVPN_MAX_RESTARTS} restarts."
            error "Exiting so Docker's restart policy can recreate the container cleanly."
            cleanup
            exit 1
        fi
    else
        # Tunnel is healthy — reset the restart counter.
        OVPN_RESTARTS=0
    fi
done
