...
# ── Pre-flight checks ─────────────────────────────────────────────────────────
require_docker() {
    command -v docker >/dev/null 2>&1 || die "Docker not installed. Run: 1proxy2xvpn setup"
    docker info >/dev/null 2>&1 || die "Docker daemon not running. Start it and try again."
}

require_haproxy() {
    command -v haproxy >/dev/null 2>&1 || die "HAProxy not installed. Run: 1proxy2xvpn setup"
}
...
print_banner() {
    cat << 'BANNER'
   ┌─────────────────────────────────────────────────┐
   │  1proxy2Xvpn — Distributed HTTP/SOCKS Proxy     │
   │  Tor-style IP rotation through OpenVPN tunnels  │
   └─────────────────────────────────────────────────┘
BANNER
}
