...
# kv <label> <value> — aligned "label ....... value" line for clean, scannable
# result blocks (Endpoints, Execution plan, etc.). Always returns 0.
kv() {
    local label="$1" value="$2" width=16
    local pad
...
# kv_secret <label> <value> — like kv but for sensitive values (indented under
# a parent). Always returns 0.
kv_sub() {
    local label="$1" value="$2" width=14
    local pad
    pad=$(printf '%*s' $(( width - ${#label} )) '')
    pad=${pad// /·}
    [ ${#label} -ge ${width} ] && pad=""
...
# Renders an in-place progress bar that fills to 100%. Prints a newline when
# current == total so following output starts on a fresh line.
progress_bar() {
    ...
    printf "\r  ${G}[%s]${N} %3d%% (%d/%d) %s" "${bar}" "${pct}" "${current}" "${total}" "${label}"
