...
install_apt() {
    ...
    # Compose: prefer the plugin, fall back to the standalone binary.
    if ! DEBIAN_FRONTEND=noninteractive apt-get install -y -qq docker-compose-plugin >/dev/null 2>&1; then
        warn "docker-compose-plugin unavailable, trying standalone docker-compose..."
        DEBIAN_FRONTEND=noninteractive apt-get install -y -qq docker-compose >/dev/null 2>&1 \
            || warn "No docker compose found — observability stack won't run until you install it"
    fi
}
