...
kv "Image"      "${IMAGE}"
kv "OVPN dir"   "${OVPN_DIR}"
kv "Total"      "${TOTAL} containers"
kv "HTTP ports" "${BASE_PORT} → ${END_PORT}"
if [ "${ENABLE_SOCKS5}" = "true" ]; then
    SOCKS_BASE=$((BASE_PORT + 10000))
    SOCKS_END=$((SOCKS_BASE + TOTAL - 1))
    kv "SOCKS5 ports" "${SOCKS_BASE} → ${SOCKS_END}"
fi
kv "Batch"      "${BATCH_SIZE} containers / ${BATCH_DELAY}s pause"
kv "Mode"       "Hardened (no --privileged)"
...
for OVPN_PATH in "${OVPN_FILES[@]}"; do
    ...
    if [ "${RUN_RC}" -eq 0 ]; then
        ...
        # Print failures on their own line (above the progress bar).
        printf "\r\033[K"  # clear the progress line before printing the error
    ...
    progress_bar "${BATCH_COUNT}" "${TOTAL}" "starting containers"
