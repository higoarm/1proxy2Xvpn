    ...
    if [ "${HAVE_ANY}" = true ]; then
        ...
        for NAME in "${!REGIONS[@]}"; do
            if [ "${REGIONS[$NAME]}" = "${REGION}" ] && [ -n "${HTTP_PORTS[$NAME]:-}" ]; then
                printf "    server %-50s 127.0.0.1:%-5s check inter 5s rise 2 fall 3\n" \
                    "${NAME}" "${HTTP_PORTS[$NAME]}"
            fi
        done
    fi
done
...
section "Endpoints"
echo "  HTTP proxy   : http://localhost:${HAPROXY_FRONTEND_PORT}"
echo "  SOCKS5 proxy : socks5://localhost:${HAPROXY_SOCKS_PORT}"
echo "  Stats        : http://${STATS_BIND}:${HAPROXY_STATS_PORT}"
echo "    User       : ${HAPROXY_USER}"
echo "    Pass       : ${HAPROXY_PASSWORD}"
...
echo
log "Test: curl -x http://localhost:${HAPROXY_FRONTEND_PORT} https://api.ipify.org"
