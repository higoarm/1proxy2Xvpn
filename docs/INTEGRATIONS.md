...
| HTTP/HTTPS with auto-retry | `http://localhost:9888` | When you need IP rotation on 403/429 (works with HTTPS/CONNECT) |
...
The smart router retries 403/429/451/503 responses, each attempt over a fresh
upstream connection so a different VPN IP is used every time. It handles both
HTTP and **HTTPS (CONNECT tunneling)**, so it works against HTTPS targets. It
also has an optional protective throttle that slows the request pace if a target
starts blocking en masse, to avoid burning the whole IP pool (disable with
`SMART_ROUTER_CIRCUIT=off`).
