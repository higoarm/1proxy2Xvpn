...
PIA uses your account username (the one starting with `p` followed by digits, like `p1234567`) and password. Create `ovpns/authvpn.txt`:
...
Then ensure each `.ovpn` contains `auth-user-pass /ovpn/authvpn.txt`.
...
Configure each `.ovpn` to use it: `auth-user-pass /ovpn/authvpn.txt`.
...
Use them in `ovpns/authvpn.txt` and reference with `auth-user-pass /ovpn/authvpn.txt`.
...
└── authvpn.txt                         ← credentials, referenced by the .ovpn files
```

Credentials live in `ovpns/` alongside the `.ovpn` files (that directory is
mounted into every container at `/ovpn`). Each `.ovpn` references them with
`auth-user-pass /ovpn/authvpn.txt`. Providers that share one username/password
across all servers (ExpressVPN, NordVPN service creds, PIA, Mullvad) can use a
single `authvpn.txt` referenced by every config. If you prefer per-provider
files, just give them distinct names and point each `.ovpn` at the right one.
...
| Error | Cause | Fix |
|-------|-------|-----|
| `AUTH_FAILED` | Bad credentials | Verify `ovpns/authvpn.txt` content |
