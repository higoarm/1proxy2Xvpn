...
Causes:
- Wrong credentials in `ovpns/authvpn.txt`
...
```bash
# Read the stats password from the local credentials file first
STATS_PASS=$(grep HAPROXY_PASSWORD ~/.config/1proxy2xvpn/credentials | cut -d= -f2)

# Then query the HAProxy stats CSV (the password never appears inline)
curl -s --user "admin:${STATS_PASS}" \
