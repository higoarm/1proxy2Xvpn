...
IPv6:
  - Fully blocked via ip6tables (INPUT/OUTPUT/FORWARD DROP, loopback only).
    The tunnel is IPv4-only, so any IPv6 traffic would bypass it and leak the
    host's real address — blocking IPv6 outright is the safe default. Also
    applied at the baseline (boot) stage to close the boot-window for IPv6.
```

**Layer 3 — Auto-restart on OpenVPN death**

The monitor loop in `entrypoint.sh` detects when OpenVPN dies and, with the Kill
Switch still active (so there is never a leak), **relaunches OpenVPN** — up to
`OVPN_MAX_RESTARTS` (default 10) attempts. If restarts are exhausted, it runs
`cleanup` and exits cleanly so Docker's restart policy recreates the container.
...
Health checks are semantic: HAProxy sends a real proxied request through each
container (to a 204 endpoint) and expects a `204`, verifying the VPN tunnel
actually works — a container whose tinyproxy is up but whose tun0 is dead fails
the check and leaves rotation. The interval is configurable via
`HAPROXY_CHECK_INTER` (default 10s) to bound overhead at scale. The container's
own Docker health check is separate and purely local (tun0 has an IP + proxy
process alive + port open), so it doesn't add tunnel traffic.

### Regional pools

**How the region is determined:** grouping is based **entirely on the `.ovpn`
filename**, not on the real geolocation of the server's IP. When containers are
discovered, the tool matches the container name (derived from the filename)
against a set of patterns and assigns each to a regional backend. No GeoIP
lookup or IP-based geolocation is performed.
...
- Name your `.ovpn` files with a recognizable country/region token (e.g.
  `us_newyork.ovpn`, `de_frankfurt.ovpn`) so they land in the right pool. Most
  providers already name their configs this way.
- A file that matches no pattern still joins the main `http_vpn_pool` (full
  rotation) — it just won't appear in a regional sub-pool.
- Because grouping is by name, a mislabeled file goes to the wrong pool. If your
  provider's filenames don't encode the region, rename them before `up`.

**Why filename and not GeoIP:** filename matching is instant, offline, and
deterministic — no dependency on an external GeoIP database or an API call per
container (which, at hundreds of containers, adds latency and a rate-limited
network dependency at startup). The trade-off is that accuracy depends on the
provider naming their files sensibly. If you need true IP-based geolocation, you
can look up each container's verified exit IP (shown in `./1proxy2xvpn status`)
against a GeoIP service out of band and rename/relabel accordingly.

Regional pools currently exist as backends ready for ACL routing; directing
traffic to a specific region requires adding an ACL or a separate frontend port
per region in the generated `haproxy.cfg`.
...
1. **HTTP and HTTPS**: handles plain HTTP and HTTPS via `CONNECT` tunneling, so it
   works with tools targeting HTTPS (Burp, sqlmap, nuclei, httpx, ffuf)
2. **Retry on status codes**: 403, 429, 451, 503 → retry up to 5 times with backoff
3. **Fresh connection per attempt/tunnel**: each retry (and each HTTPS tunnel)
   uses a new upstream connection, so `balance roundrobin` advances to a
   different VPN IP every time (no keep-alive pinning the IP)
4. **Protective throttle (circuit breaker)**: if the failure rate crosses a
   threshold, it slows the pace of new requests to avoid burning the whole IP
   pool. Reversible, never removes a backend, conservative by default, and
   disableable via `SMART_ROUTER_CIRCUIT=off`
5. **Prometheus metrics** exposed at `:9888/metrics` (including burn rate and
   throttle state)
6. **Standard library only** — no third-party dependency for the proxy itself
   (`prometheus_client` is optional, just for `/metrics`)
...
- Against HTTPS targets that need retry-with-rotation
...
- For heavy fuzzing where many 4xx are expected by design — either raise the
  circuit-breaker threshold or run with `SMART_ROUTER_CIRCUIT=off`
...
|---------|-----------|--------|----------|
| OpenVPN crashes | Monitor loop in entrypoint | Container has no internet | Kill Switch active (no leak); OpenVPN auto-relaunched, up to 10 attempts, then clean exit for Docker restart |
